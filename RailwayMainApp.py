...
# goToPayment Window
def goToPayment(trainData, formDetails, passengerData):
    global gtp
    gtp = Tk()
    pst.iconify()
    gtp.title("Payment")
    ...
    def confirmTicket():
        ...
        # insert into table
        c.execute('INSERT INTO TICKETS VALUES(:tkno,:pname,:pemail,:tno,:froms,:tos,:tdate,:typ,:notkts,:fare)',
        {
                'tkno': tn,
        ...
        c.execute("UPDATE TICKETS SET TKTID=OID")
    ...
    buttonConfirm = Button(gtp, text = "Confirm", background = "black", foreground = "grey", command=confirmTicket)
...
def showTrains(trainsData, formDetails, passengerData):
    ...
    for i in range (len(trainsData)):
        ...
        sampleButton = Button(second_frame,text="Book Now",fg="white",bg="black",command=lambda:goToPayment(trainsData[i-1],formDetails,passengerData))
...
def adminHome(email):
    ...
    transaction = Button(ah, text="Transactions", background = "black", foreground = "grey", pady = 20, command = showTransaction)
